...
#
# Generated on 2026-08-08T00:53:49Z by kres 93d28ca.
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg go install github.com/shanduur/dis-vulncheck@${DIS_VULNCHECK_VERSION} \
...
WORKDIR /src
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg dis-vulncheck -tool=false ./...
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg GOARCH=amd64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /siderolink-agent-darwin-amd64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg GOARCH=arm64 GOOS=darwin go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /siderolink-agent-darwin-arm64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /siderolink-agent-linux-amd64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /siderolink-agent-linux-arm64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg GOARCH=arm GOARM=7 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /siderolink-agent-linux-armv7
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=siderolink/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=siderolink/go/pkg GOARCH=amd64 GOOS=windows go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /siderolink-agent-windows-amd64.exe
