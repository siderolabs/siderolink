...
func (m *ProvisionResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.NodeAddressPrefix) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.NodeAddressPrefix)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.ServerPublicKey) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ServerPublicKey)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.ServerEndpoint) > 0 {
		for iNdEx := len(m.ServerEndpoint) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ServerEndpoint[iNdEx])))
			i--
			dAtA[i] = 0xa
		}
	}
